...
func Register() {
	...
	h.Handle("/go_sword_public/", http.StripPrefix("/go_sword_public/",
		http.FileServer(http.Dir("admin/resource/dist"))))
	// Default index.html
	...
	// Route tag tunnel
	h.HandleFunc("/api/tunnel/list", handleError(tunnel.List))
	h.HandleFunc("/api/tunnel/delete", handleError(tunnel.Delete))
	h.HandleFunc("/api/tunnel/detail", handleError(tunnel.Detail))
	h.HandleFunc("/api/tunnel/create", handleError(tunnel.Create))
	h.HandleFunc("/api/tunnel/edit", handleError(tunnel.Edit))
	h.HandleFunc("/api/tunnel/batch_delete", handleError(tunnel.BatchDelete))
	// ----Route-end----
...
func render(writer http.ResponseWriter, request *http.Request) {
	...
	// 从view目录中寻找文件
	body := readFile("admin/view" + path + ".html")
